...
*&---------------------------------------------------------------------*
FORM fill_report .
  TYPES: BEGIN OF ltt_daily_sales,
         file_date               TYPE zsg_t_019-file_date             ,
         gjahr                   TYPE zsg_t_019-gjahr                 ,
         monat                   TYPE zsg_t_019-monat                 ,
         retailer_no             TYPE zsg_t_019-retailer_no           ,
         sales_amount            TYPE zsg_t_019-sales_amount          ,
         cancelled_sales_amount  TYPE zsg_t_019-cancelled_sales_amount,
         retailer_name           TYPE zsog_fi_004_t_01-retailer_name  ,
         katr2                   TYPE kna1-katr2,
         katr3                   TYPE kna1-katr3,
         katr4                   TYPE kna1-katr4,
         katr5                   TYPE kna1-katr5,
         katr6                   TYPE kna1-katr6,
         END OF ltt_daily_sales,
         BEGIN OF ltt_payout_stat,
         file_date               TYPE zsg_t_020-file_date           ,
         gjahr                   TYPE zsg_t_020-gjahr               ,
         monat                   TYPE zsg_t_020-monat               ,
         retailer_no             TYPE zsg_t_020-retailer_no         ,
         payout_amount           TYPE zsg_t_020-payout_amount       ,
         cncld_payout_amount     TYPE zsg_t_020-cncld_payout_amount ,
         refund_amount           TYPE zsg_t_020-refund_amount       ,
         cncld_refund_amount     TYPE zsg_t_020-cncld_refund_amount ,
         sum_packet_no           TYPE zsg_t_017-sum_packet_no       ,
         sum_group               TYPE zsg_t_017-sum_group           ,
         commission_rate         TYPE zsg_t_017-commission_rate           ,
         END OF ltt_payout_stat.

  DATA:  lt_daily_sales     TYPE TABLE OF ltt_daily_sales,
         ls_daily_sales     TYPE ltt_daily_sales,
         lt_payout_stat     TYPE TABLE OF ltt_payout_stat,
         ls_payout_stat     TYPE ltt_payout_stat,
         lt_tax             TYPE TABLE OF zsog_fi_004_t_04,
         ls_tax             TYPE zsog_fi_004_t_04,
         ls_alv             TYPE zsog_fi_004_t_01.
  ...
  SELECT z1~file_date
         ...
         z1~retailer_no
         z1~sales_amount
         z1~cancelled_sales_amount
         k~name1 AS retailer_name
         k~katr2
         k~katr3
         k~katr4
         k~katr5
         k~katr6
         FROM zsg_t_019 AS z1
         LEFT OUTER JOIN kna1 AS k ON k~kunnr = z1~retailer_no
         INTO TABLE lt_daily_sales
         WHERE z1~file_date IN s_tarih.
  ...
  SELECT z1~file_date
         ...
         z1~retailer_no
         z1~payout_amount
         z1~cncld_payout_amount
         z1~refund_amount
         z1~cncld_refund_amount
         z2~sum_packet_no
         z2~sum_group
         z2~commission_rate
         FROM zsg_t_020 AS z1
         LEFT OUTER JOIN zsg_t_017 AS z2 ON z1~file_date = z2~file_date
                                        AND z2~retailer_no = z2~retailer_no
         INTO TABLE lt_payout_stat
         FOR ALL ENTRIES IN lt_daily_sales
         WHERE z1~file_date   = lt_daily_sales-file_date
           AND z1~retailer_no = lt_daily_sales-retailer_no.

  SELECT * FROM zsog_fi_004_t_04 INTO TABLE lt_tax.

  SORT lt_payout_stat BY file_date retailer_no.

  LOOP AT lt_daily_sales INTO ls_daily_sales.
    READ TABLE lt_payout_stat INTO ls_payout_stat WITH KEY file_date = ls_daily_sales-file_date
                                                           retailer_no = ls_daily_sales-retailer_no
                                                           BINARY SEARCH.
    ls_alv-retailer_no          = ls_daily_sales-retailer_no.
    ls_alv-retailer_name        = ls_daily_sales-retailer_name.
    ls_alv-gross_sales          = ls_daily_sales-sales_amount - ls_daily_sales-cancelled_sales_amount.
*    ls_alv-AMOUNT_TO_BE_REFUND  =
    ls_alv-paid_refunds         = ls_payout_stat-refund_amount - ls_payout_stat-cncld_refund_amount.
    ls_alv-gross_sales_refunds  =  ls_alv-gross_sales  - ls_alv-amount_to_be_refund.


*    ls_alv-NET_PAYOUT
*    ls_alv-INHERITANCE_TAX
*    ls_alv-INTERIM_PAYMENT

    READ TABLE lt_tax INTO ls_tax WITH KEY tax_type = '01'.
*    ls_alv-gross_sales_vat = ( ls_alv-gross_sales / ( 1 + ls_tax-rate ) ) * ls_tax-rate.
    CLEAR: ls_tax.
    READ TABLE lt_tax INTO ls_tax WITH KEY tax_type = '02'.
*    ls_alv-gross_sales_sov =  ( ls_alv-gross_sales - ls_alv-gross_sales_vat ) * ls_tax-rate .
    CLEAR: ls_tax.
*    ls_alv-gross_sales_tax_free =  ls_alv-gross_sales - ls_alv-gross_sales_vat - ls_alv-gross_sales_sov.

    READ TABLE lt_tax INTO ls_tax WITH KEY tax_type = '01'.
*    ls_alv-amount_to_be_refund_vat = ( ls_alv-amount_to_be_refund / ( 1 + ls_tax-rate ) ) * ls_tax-rate.
    CLEAR: ls_tax.

    READ TABLE lt_tax INTO ls_tax WITH KEY tax_type = '02'.
*    ls_alv-amount_to_be_refund_sov =  ( ls_alv-amount_to_be_refund - ls_alv-amount_to_be_refund_vat ) * ls_tax-rate .
    CLEAR: ls_tax.
**    ls_alv-AMOUNT_TO_BE_REFUND_SOV
*    ls_alv-amount_to_be_refund_tax_free = ls_alv-amount_to_be_refund  - ls_alv-amount_to_be_refund_vat - ls_alv-gross_sales_sov.
*    ls_alv-net_sales                    = ls_alv-gross_sales_tax_free - ls_alv-amount_to_be_refund_tax_free.
*    ls_alv-sov                          = ls_alv-gross_sales_sov - ls_alv-amount_to_be_refund_sov.
*    ls_alv-vat                          = ls_alv-gross_sales_vat - ls_alv-amount_to_be_refund_vat.
*    ls_alv-sales_comission              = ls_alv-gross_sales_tax_free  * ls_payout_stat-commission_rate.
*    ls_alv-pbc_comission                = ls_alv-gross_sales_tax_free  * ls_payout_stat-commission_rate.
*    ls_alv-comission_to_refund          = ls_alv-amount_to_be_refund_tax_free * ls_payout_stat-commission_rate.
*    ls_alv-payout                       = ls_payout_stat-payout_amount - ls_payout_stat-cncld_payout_amount.
*    ls_alv-payout_comission             = ls_alv-payout * ls_payout_stat-commission_rate.
**    ls_alv-NET_COMISSION
*    ls_alv-katr2   = ls_daily_sales-katr2.
*    ls_alv-katr3   = ls_daily_sales-katr3.
*    ls_alv-katr4   = ls_daily_sales-katr4.
*    ls_alv-katr5   = ls_daily_sales-katr5.
*    ls_alv-katr6   = ls_daily_sales-katr6.

    READ TABLE lt_tax INTO ls_tax WITH KEY tax_type = '03'.
    ls_alv-income_tax = ls_payout_stat-commission_rate * ls_tax-rate.

    APPEND ls_alv TO gs_scr-1903-alv.
    CLEAR: ls_payout_stat, ls_alv, ls_tax.
  ENDLOOP.
