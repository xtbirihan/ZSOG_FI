...
FORM create_tables .
  ...
  LOOP AT <rlf_tab_d> ASSIGNING <rlf_line_d>.

    MOVE-CORRESPONDING <rlf_line_d> TO ls_alv.
    MOVE-CORRESPONDING <rlf_line_d> TO ls_alv.

*added by ilknurnacar 210819
    IF ls_alv-email EQ '0' .
      CLEAR ls_alv-email.
    ENDIF.
*end of added by ilknurnacar 210819
    ...
    IF ls_alv-retailer_debtor_id IS NOT INITIAL.
      ...
      LOOP AT <rmf_tab_d> ASSIGNING <rmf_line_d> WHERE (lv_cond).
        MOVE-CORRESPONDING <rmf_line_h> TO ls_rmf_alv.
        MOVE-CORRESPONDING <rmf_line_d> TO ls_rmf.
    ...
    ELSE.
      ls_alv-rmf_icon = icon_system_stop_recording.
    ENDIF.
    IF ls_alv-retail_location_id IS NOT INITIAL.
    ...
    CLEAR: ls_alv, lv_cond, lt_rmf, lv_cond2.
  ENDLOOP.
  PERFORM modify_data.
